...
yum -y install docker-engine-1.10.2-1.el7.centos.x86_64

# Docker installation
sed -i -e "s%^\[Service\].*%\[Service\]\nEnvironmentFile=-/etc/sysconfig/docker%g" /usr/lib/systemd/system/docker.service
sed -i -e "s%^ExecStart=.*%ExecStart=/usr/bin/docker -d -H fd:// \$other_args%g" /usr/lib/systemd/system/docker.service
