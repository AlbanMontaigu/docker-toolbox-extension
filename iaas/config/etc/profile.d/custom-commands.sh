...
# --------------------------------------
alias dk-devbox-run='docker run -it -v /vagrant:/vagrant -v /etc/dk-devbox/.zshrc:/home/dev/.zshrc -v /var/run/docker.sock:/var/run/docker.sock -e COMPOSE_PROJECT_NAME=${COMPOSE_PROJECT_NAME:-app} --name="dk-devbox" amontaigu/docker-devbox:1.9.0'
...
# ------------------------------------------------------------
alias dk-redsocks-run='docker run --privileged=true --net=host -v /etc/dk-redsocks/whitelist.txt:/etc/redsocks-whitelist.txt --name="dk-redsocks" -d amontaigu/redsocks PUT_YOUR_PROXY_HERE 3128'
